...
def getTokenImageURL(_token):
    ...
    try:
        r = reqSess.get(url=_url,timeout=10) 
        usr = r.json()
        return usr['image'].replace('ipfs://','https://ipfs.io/ipfs/')
    except Exception as e:
        return 0
...
_maxTokens = warlineTokenContract.functions.maxTokens().call() 

_oldID = -1
while (True):
    time.sleep(1)
    _currentID = warlineTokenContract.functions.totalSupply().call()
    if _currentID >= _maxTokens:
        print("Minting concluded, all tokens minted")
        break
